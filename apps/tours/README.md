...
| POST | `/traslados` | Registra un traslado (D-34): asiento caja/ingresos + deuda con el proveedor de transporte, en cuentas propias. Dos fechas: cobro (contable) y servicio. No entra en liquidaciones | admin/contabilidad/vendedor (propias) |
...
- **D-34** Traslados como segunda línea de negocio sobre `tours_servicios`, discriminada por `tipo_servicio`: reusa el asiento balanceado, la edición/borrado y el audit trail en vez de duplicar el circuito. El destino es texto libre (`tour_id` apunta a la fila de catálogo genérica `SRV-TRASLADO`). **El hotel somos nosotros**, así que no hay tercero a quien acreditarle nada: el traslado es una venta simple y el margen (`monto − costo`) queda como resultado. Ingresos, costos y deuda van en cuentas propias (`401`/`501-TRASLADOS`, `203-TRANSPORTE-POR-PAGAR`) para leer cada línea por separado. `agencias.tipo` separa `proveedor_tour` de `proveedor_transporte` y los formularios no mezclan las listas. Dos fechas explícitas: `fecha` es la de cobro —la que fecha el asiento— y `fecha_servicio` la del traslado. Los traslados no comisionan al vendedor y por eso quedan fuera de las liquidaciones.
