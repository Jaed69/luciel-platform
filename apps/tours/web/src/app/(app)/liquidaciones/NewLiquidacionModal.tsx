...
export function NewLiquidacionModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  const columns: Column<Candidata>[] = [
    ...
    { key: "vendedor", header: "Vendedor", render: (r) => vendedorNombre(r.vendedor_id) },
    { key: "agencia", header: "Agencia", render: (r) => agenciaNombre(r.agencia_id) },
    { key: "monto", header: "Monto", render: (r) => <span className="tabular-nums">{formatCurrency(r.monto, r.moneda)}</span> },
    {
      key: "observaciones",
      header: "Notas",
  ...
  return (
    <Modal open={open} onClose={onClose} maxW={step === 1 ? "md" : "xl"}>
      {step === 1 ? (
        ...
        <>
          <h2 className="font-playfair text-primary text-2xl font-semibold mb-1">Elegí qué ventas liquidar</h2>
          ...
          <DataTable columns={columns} data={candidatas} emptyState="No hay ventas sin liquidar en este rango." />
